function [convergence_data, distribution_data, p_n_indyk, predictions,labels_unique] = indyk_wrapper(hierarchy,connectivity,labels, no_of_iterations,k, unknownstr)
...
for i=1:no_of_nodes
    ...
    for j=1:no_of_nodes
        ...
        %issibling
        if(find(hierarchy(:,j),1)==find(hierarchy(:,i),1))
...
for i=1:no_of_iterations
    distribution_next=collective_inference(distribution_cur,hierarchy,connectivity,labelcodes,labels_unique,k,p_n_indyk,p_labels);
